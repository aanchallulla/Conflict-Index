...
##Dept fixed effects
dept_model1 <- feols(
  c(s11_score_total, s11_score_math, s11_score_english, net_cov_edu, gross_cov_edu, stud_teach_ratio) ~ AMPI | dept_code, 
                     data = merged, 
                     cluster = ~dept_code)
summary(dept_model1)

##Dept and year fixed effects
dept_model2 <- feols(
  c(s11_score_total, s11_score_math, s11_score_english, net_cov_edu, gross_cov_edu, stud_teach_ratio) ~ AMPI | dept_code + ano, 
  data = merged, 
  cluster = ~dept_code)
summary(dept_model2)

##Dept and year fixed effects with controls
dept_model3 <- feols(
  c(s11_score_total, s11_score_math, s11_score_english, net_cov_edu, gross_cov_edu, stud_teach_ratio) ~ AMPI + rural_i + effect_i + efficien_i| dept_code + ano, 
  data = merged, 
  cluster = ~dept_code)
summary(dept_model3)
